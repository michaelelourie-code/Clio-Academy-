...
/* ---------- CONTACT ---------- */
// Bookings are emailed to clionotes.academy@gmail.com via Formspree's
// official React SDK — https://formspree.io/f/mvzjnldd

function Contact() {
  const [state, handleSubmit, reset] = useForm("mvzjnldd");
            ...
            <div className="form">
              {state.succeeded ? (
                <div className="sent">
                  ...
                  <div style={{ marginTop: 18 }}><Btn variant="ghost" onClick={() => reset()}>Send another</Btn></div>
                </div>
              ) : (
                <form onSubmit={handleSubmit}>
                  <input type="hidden" name="_subject" value="Clio Academy consultation request" />
                  <div className="two">
                    <div className="field">
                  ...
                  <div className="two">
                    <div className="field">
                      <label htmlFor="cf-role">You are a</label>
                      <select id="cf-role" name="role" defaultValue="Parent">
                        <option>Parent</option><option>Student</option><option>Other</option>
                      </select>
                    </div>
                    <div className="field">
                      <label htmlFor="cf-year">Year group</label>
                      <select id="cf-year" name="year_group" defaultValue="Lower Sixth">
                        <option>Lower Sixth</option><option>Upper Sixth</option><option>GCSE (taster)</option><option>Other</option>
                      </select>
                    </div>
                  </div>
                  <div className="field">
                    <label htmlFor="cf-school">School</label>
                    <input id="cf-school" name="school" placeholder="School name" />
                  </div>
                  <div className="field">
                    <label htmlFor="cf-topics">OCR topics studied</label>
                    <input id="cf-topics" name="ocr_topics" placeholder="e.g. Athens · The Julio-Claudians" />
                  </div>
                  <div className="field">
                    <label htmlFor="cf-concerns">Current concerns / goals</label>
                    <textarea id="cf-concerns" name="concerns" placeholder="What would you like to improve?" />
                  </div>
                  <div className="field">
                    <label htmlFor="cf-time">Preferred session time</label>
                    <input id="cf-time" name="preferred_time" placeholder="e.g. weekday evenings" />
                  </div>
                  <button className="btn btn--gold" style={{ width: "100%", justifyContent: "center" }} type="submit" disabled={state.submitting}>
                    {state.submitting ? "Sending…" : "Request a consultation"}<ArrowRight />
                  </button>
